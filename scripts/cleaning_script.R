...
austria_total_source_latest <- fread("source-data/austria/austria_total_source_latest.csv") 
...
belgium_weekly_covid_deaths <- belgium_covid_source_latest %>%
  ...
  mutate(country = "Belgium",
         week_date = date + 2, # Use Belgian weekly windows
         week = week(week_date),
         year = year(week_date)) %>%
...
# Step 4: import and clean Britain's data---------------------------------------
...
# Step 5: import and clean Chile's data ---------------------------------------
...
chile_regions_weekly_total_deaths <- bind_rows(chile_total_source_2015_12_31,chile_total_source_2016_12_31,
  ...
  mutate(year = AÑO, month = MES, day = DIA, region = REGION) %>%
...
# Step 6: import and clean Denmark's data---------------------------------------
...
denmark_weekly_deaths <- denmark_weekly_total_deaths %>%
  ...
  drop_na() %>%
  filter(week != 53,
         end_date <= as.Date("2020-05-05")) # Remove weeks with incomplete data
...
# Step 7: import and clean Ecuador's data---------------------------------------
...
# Step 8: import and clean France's data---------------------------------------
...
france_covid_source_latest <- fread("https://raw.githubusercontent.com/opencovid19-fr/data/master/data-sources/sante-publique-france/covid_hospit.csv")
...
write.csv(bind_rows(france_regions_weekly_deaths,france_national_weekly_deaths) %>%
            mutate(start_date = format(start_date, "%Y-%m-%d"),
                   end_date = format(end_date, "%Y-%m-%d")) %>%
            filter(end_date <= as.Date("2020-04-28")), # Remove weeks with incomplete data
          "output-data/historical-deaths/france_weekly_deaths.csv",
          fileEncoding = "UTF-8",
          row.names=FALSE)

# Step 9: import and clean Germany's data---------------------------------------
...
germany_weekly_deaths <- germany_weekly_total_deaths %>%
  ...
  drop_na() %>%
  filter(week != 53,
         end_date <= as.Date("2020-04-14")) # Remove weeks with incomplete data
...
# Step 10: import and clean Indonesia's data---------------------------------------
...
# Step 11: import and clean Italy's data ---------------------------------------

# Import Italy's data
italy_comunes <- read_excel("source-data/italy/italy_comunes.xlsx")
italy_total_source_latest <- fread("source-data/italy/italy_total_source_2020_04_15.csv")
...
italy_comunes_reliable <- italy_total_source_latest %>%
  filter(GE <= 331, T_20 != "n.d.") %>% # Filter out any comunes missing data before March 31st
...
# Step 12: import and clean the Netherlands' data---------------------------------------
...
# Step 13: import and clean Norway's data---------------------------------------
...
# Step 14: import and clean Portugal's data---------------------------------------
...
# Step 15: import and clean Russia's data---------------------------------------
...
# Step 16: import and clean South Africa's data---------------------------------------
...
# Step 17: import and clean Spain's data---------------------------------------
...
spain_regions_weekly_deaths <- spain_regions_weekly_total_deaths %>%
  ...
  dplyr::select(country,region,region_code,start_date,end_date,year,week,
                population,total_deaths,covid_deaths,expected_deaths) %>%
  drop_na() %>%
  filter(end_date <= as.Date("2020-05-05")) # Remove weeks with incomplete data
...
# Step 18: import and clean Sweden's data---------------------------------------
...
sweden_weekly_deaths <- sweden_weekly_total_deaths %>%
  ...
  drop_na() %>%
  filter(week != 53,
         end_date <= as.Date("2020-04-28")) # Remove weeks with incomplete data
...
# Step 19: import and clean Switzerland's data---------------------------------------
...
# Step 20: import and clean Turkey's data---------------------------------------
...
# Step 21: import and clean the United States' data---------------------------------------
...
united_states_weekly_deaths <- united_states_weekly_total_deaths %>%
  ...
  dplyr::select(country,region,region_code,start_date,end_date,year,week,
                population,total_deaths,covid_deaths,expected_deaths) %>%
  drop_na() %>%
  filter(end_date <= as.Date("2020-04-25")) # Remove weeks with incomplete data
